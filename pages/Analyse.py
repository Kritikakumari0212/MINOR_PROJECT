...
fig = go.Figure()
fig.add_trace(go.Scatter(x=data['time'], y=data['generation solar'], line=dict(color='orange')))
fig.update_layout(
    xaxis_title='Time',
    yaxis_title='Generation (MW)',
...
fig_line_dual.add_trace(go.Scatter(x=data['time'], y=data['generation nuclear'], 
                                   mode='lines', line=dict(color='purple'), name='Nuclear Generation (MW)'))

fig_line_dual.add_trace(go.Scatter(x=data['time'], y=data['generation fossil gas'], 
